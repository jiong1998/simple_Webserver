...
int pthread_number=1024;//允许最大连接的客户数
struct Pthread_Struct pthread_struct[1024];
//初始化线程结构体数组
...
int main()
{
    ...
    while(1)
    {
        ...
        for(i=0;i<nready;++i)
        {
            ...
            //2、客户的数据到,使用线程处理客户的数据请求
            else
            {
                connfd = event[i].data.fd;
                //用线程结构体数组接收connfd
                k = find_index(pthread_struct);
                //对空闲位置的元素的成员赋值
                pthread_struct[k].connfd=connfd;
                pthread_struct[k].epfd = epfd;
                //创造子线程进行通信
                pthread_create(&(pthread_struct[k].pthreadID), NULL, http_request, &(pthread_struct[k]));
                //设置子线程为分离属性
                pthread_detach(pthread_struct[k].pthreadID);
            }
        }
    }
    close(listenfd);
